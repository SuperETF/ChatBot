...
router.post("/", async (req, res) => {
  ...
  try {
    // ✅ 전문가 등록 (이름 + 번호 + 인증번호)
    if (/^전문가\s+[가-힣]{2,10}\s+01[016789][0-9]{7,8}\s+\d{4}$/.test(utterance)) {
      return auth.auth(kakaoId, utterance, res, "registerTrainer");
    }

    // ✅ 트레이너가 회원 등록 (이름 + 번호 + 인증번호)
    if (/^회원\s+[가-힣]{2,10}\s+01[016789][0-9]{7,8}\s+\d{4}$/.test(utterance)) {
      return auth.auth(kakaoId, utterance, res, "registerTrainerMember");
    }

    // ✅ 회원 본인 등록 (이름 + 번호 + 인증번호)
    if (/^[가-힣]{2,10}\s+01[016789][0-9]{7,8}\s+\d{4}$/.test(utterance)) {
